...
public class TranscriberClient {
  ...
  private final TextTranscriber textTranscriber;
  ...
  public TranscriberClient(JDA jda) {
    ...
    this.textTranscriber = new TextTranscriber();
  ...
  /**
   * Provides a {@link CompletableFuture} of the text transcript generation task for the provided {@code channel}.
   *
   * @param channel
   *   the {@link GuildMessageChannel} to transcribe.
   *
   * @return {@link CompletableFuture} of a {@link TextTranscript}.
   */
  public CompletableFuture<TextTranscript> transcribeText(GuildMessageChannel channel) {
    return transcribeText(channel, new PayloadOptions());
  }
   ...
   * @return {@link CompletableFuture} of a {@link TextTranscript}.
   */
  public CompletableFuture<TextTranscript> transcribeText(GuildMessageChannel channel, PayloadOptions options) {
    try {
      TranscriberUtil.checkChannel(channel);
    } catch (TranscriberPermissionException e) {
      return CompletableFuture.failedFuture(e);
    ...
    return CompletableFuture
      ...
      .thenApply(v -> new Payload(
        ...
        messagesFuture.join(),
        options))
      .thenApply(payload -> textTranscriber.transcribe(payload, options));
  }
}
